...
description: Every event opx77_hud listens to — the core's local character events, the status strip payload, and its own /hud net event — plus the complete message protocol between its Lua half and web/hud.js.
---
...
character is read from `opx77_core` through a client export, and the status chips
arrive on the client's local bus.
...
!!! warning "Treated as untrusted input"
    ...
    most **12** chips from one payload, drops any chip without an `id`, and
    coerces `hidden` to a number defaulting to `0`. The chip id `__more` is
    reserved for the overflow counter the page draws and a raw publisher should
    not mint it.
...
- tween: `integer` — **sent and currently read by nothing in the shipped page.**
  The 220 ms bar animation is a CSS transition.
...
- rows: `table[]` — built in `OPX_HUD_CONFIG.BLOCKS` order. Two shapes:
    ...
    | `label` | present, **not drawn** | drawn as the line's label |
...
A frame sent while no character is loaded but a chip is live also carries an
empty `info` table, which the page ignores.

!!! warning "A redraw that moves nothing sends nothing"
    Before each frame the rows are reduced to a signature — id, label, value,
    percent, tone and icon of every row, plus the chips' ids, labels and tones,
    `hidden`, `anchor` and `offset`. If it matches the last frame sent, nothing is
    sent. `remainingMs` is deliberately absent from it: a countdown ticking down
    is not a new picture, and the page animates it on its own clock. The signature
    is forced only on `hud:ready` and on a visibility change, because neither is
    described by it.
