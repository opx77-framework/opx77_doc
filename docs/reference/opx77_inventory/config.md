...
description: Every key of OPX_INVENTORY_CONFIG with its shipped default and accepted range — the language, the core, bag and vehicle sizes, reach, the hotbar and the keys, use and rate limits, saving, piles on the ground, stashes, weapons, the tabs, the export callers and the staff commands.
---
...
logs, the console's command answers and the error codes stay English.
...
| Key | Range | Does |
|---|---|---|
| `DEFAULT_ITEM_WEIGHT` | grams | the weight of a stored item the catalogue no longer carries |
...
| Key | Range | Does |
|---|---|---|
| `DISTANCE` | `0.5`..`25` m | to a stash, a pile on the ground, or a player handed something |
| `VEHICLE` | `1`..`25` m | from a vehicle's centre, to reach its trunk |
...
from the hotbar keys, the screen closed. Only `false` turns it off.
...
`OPEN` shipped as `F7` before; it is `I` now, which is also the game's own
backpack key — see [Keys](index.md#keys) for what that costs and how the page
closes on it.
...
| `DISTANCE` | `0.1`..`10` m | a drop this close to a pile goes in that pile |
| `MODEL` | | the world prop drawn for a pile: a curated `Open77.props` alias; `prop.catalog` in game lists them |
| `PROMPT_KEY` | key name | the key the pile's prompt answers to |
...
`SLOTS` is `0`..`200` and `MAX_WEIGHT` `0`..`4000000000` grams. A record
containing one of `BIKES.PATTERNS` is a two-wheeler: a trunk divided by
`TRUNK_DIVISOR` (`1`..`100`), and no glovebox. An owned vehicle keeps its storage
on its plate; any other vehicle's lives as long as the vehicle does.
...
| `LABEL` | a catalogue key or plain text |
| `SLOTS` | `1`..`200`, default `50` |
| `MAX_WEIGHT` | grams, default `100000` |
| `POSITION` | `X`, `Y` and `Z`, required |
| `BUCKET` | the routing bucket, default `0` |
...
`CATEGORIES` gathers item categories; `REST = true` takes the leftovers. A tab
with no `KEY` is left out with a warning.
...
How long a refusal or a notice stays on screen, `750`..`120000` ms.
...
(`OPX_INVENTORY_WEAPONS`) are definitions, not settings. Nothing is created in
game. An item's key is the name stored in every container: renaming one strands
what players hold, and an item taken out of the file stays where it is, unusable,
until somebody moves it. A malformed entry is named at boot:
