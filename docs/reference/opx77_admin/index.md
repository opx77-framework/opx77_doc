...
description: opx77_admin is the staff tool for OPX//77 — forty-three restricted commands and a keyboard menu that drives them, gated by acl.jsonc and nothing else, with every action audited in the platform log.
---
...
| **Commands** | forty-three, **every one** ACL-restricted — see [Commands](commands.md) |
| **Events** | eight server-to-client net events and one client-to-server, all private — see [Events](events.md) |
| **Optional at runtime** | `opx77_menu`, `opx77_input`, `opx77_notify`, `opx77_core`, `opx77_appearance`, `opx77_weather`, [`opx77_inventory`](../opx77_inventory/index.md), and the platform's `open77_weapons` |
...
character *carries*, weapons included, belongs to
...
Open77.weapons is unavailable on this host: holster refuses, and a refill writes the drawn weapon's item without loading it in hand
```
...
```text
ready -- 43 restricted commands; grant command.opx77.admin to open the menu
```

## Nothing touches a body behind a closed gate {#readiness-gate}

Every teleport, kill, heal, god toggle, health or armour write, holster and
vehicle delivery first needs **both** a life state — the "continue" screen
...
bag — a weapon or inventory command other than the holster — which
`opx77_inventory` makes in its own records and pushes when the player is there
to see it. A refill of the weapon a player has drawn is loaded in hand only for
a player the gate admits; for anybody else only its item is written.
...
- **A target is a player id, or `me`.** `self` is read as `me`. A command that
  acts on a body takes nothing else. The weapon and inventory commands but the
  holster take a **holder** instead: a player id, `me`, or a citizen id, which
  also reaches a character that is not in the world — `opx77_inventory`
  resolves it through the core and loads that bag for the call. See
  [Targets](commands.md#targets).
...
|---|---|---|
| [`weapon.give`](commands.md#weapon-give) | `CanCarry`, then `AddItem(holder, weapon, 1, { ammo = rounds })` | `command.opx77.admin.weapon.give` |
| [`weapon.ammo`](commands.md#weapon-ammo) | `GetInventory`, `GetHeldWeapon`, `SetMetadata` per weapon item; the drawn one's rounds also through the relay's `setAmmo` first | `command.opx77.admin.weapon.ammo` |
...
!!! warning "Three grants create things"

    `command.opx77.admin.inventory.give` and `command.opx77.inventory.give` both
    create items out of nothing, and `command.opx77.admin.weapon.give` creates
    weapons. Grant them like money.
...
changes no item and leaves the weapon in its slot. A refill of the weapon the
player has drawn states its new rounds to the engine with `setAmmo` before its
item is written, because the inventory lowers a drawn weapon's item to what the
engine reads back and never raises it: an item written first would be lowered
...
- `message` is the English detail — the coordinates of a move, the item, holder
  and rounds of a weapon give, the reason of a kick — cut to 120 characters, with
...
audited; the gate's refusals, a native's, a failed placement and every refusal
that comes back from `opx77_inventory` are. Reading a bag with
...
| File | Does |
|---|---|
| `config.lua` | shared. Rates, placement, noclip, announcements, vehicles, the inventory, the linked commands, the sky lists, the destinations |
| `data/vehicles.lua` | shared. The vehicle catalogue, and the allowlist |
| `data/weapons.lua` | shared. The weapon classes only: their menu order, their label and the rounds a give loads |
...
| `server/weapons.lua` | give, refill, take and read weapon items through `server/inventory.lua`; the holster and a drawn weapon's rounds over the platform's weapon relay |
...
- **The weapon rows** — give, refill, holster, take every weapon, weapons in the
  bag — are drawn for yourself and for a player. Every one but the holster is
  ...
  order, then any class it does not name, under its key. A picker whose list has
  not arrived shows *Loading...*, and *opx77_inventory did not answer.* when it
  could not be had.
...
| A full vehicle catalogue | No enumeration binding exists, and shipping a generated list is its own maintenance. Weapons and items are `opx77_inventory`'s catalogue. |
...
- [Commands](commands.md) — all forty-three, their arguments, their refusals,
  and the permissions to grant.
- [Configuration](config.md) — every key of `OPX_ADMIN_CONFIG`, the two
  data files and the locale catalogue.
