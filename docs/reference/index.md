...
| [`opx77_menu`](opx77_menu/index.md) | 0.4.0 | `reconnect` | Shared keyboard-driven menu service. |
| [`opx77_input`](opx77_input/index.md) | 0.1.0 | `reconnect` | The one form on the client: text, choice and slider fields answered together, drawn as `opx77_menu`'s strip. |
...
- **`opx77_chat`, `opx77_weather`, `opx77_notify` and `opx77_input` depend on
  nothing else in the set.** `opx77_notify` is the one resource whose main
...
a userdata, so it is tested for presence and never for being a table. The full
