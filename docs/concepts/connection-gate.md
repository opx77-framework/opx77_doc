...
actual instant, neither needing a permission:
...
`opx77_core` does request `players.disconnect`, for one purpose: the three
paths where it cannot bring a player in — no verified identity, the roster query
failed, nobody chose a character in time. Releasing the readiness gate on those
paths, which is what it used to do, drops the player into the world with no
character and no way to get one. `players.ban` is not requested and should not
